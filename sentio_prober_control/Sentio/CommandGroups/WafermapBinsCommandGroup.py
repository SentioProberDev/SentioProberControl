...
class WafermapBinsCommandGroup(CommandGroupBase):
    ...
    def clear_all(self) -> None:
        """Clear all bins. Remove the bin code from all dies and subsites."""
    ...
    def get_bin(self, col: int | None = None, row: int | None = None, site: int | None = None) -> int:
        ...
        if col is None and row is None and site is None:
            self.comm.send("map:bins:get_bin")
            ...
            raise ValueError("get_bin command requires either no parameter, column and row, or column, row, and site.")
    ...
    def get_bin_info(self, bin: int) -> Tuple[int, str, BinQuality, str]:
        ...
        Returns:
            A tuple containing the following data items: bin index (same value as the argument), bin description,
            bin quality (pass/fail information), and bin color.
        """
    ...
    def set_bin(self, bin_value: int, col: int | None = None, row: int | None = None, site: int | None = None) -> None:
        ...
        if col is None and row is None and site is None:
            ...
            raise ValueError("set_bin command requires either no parameter, column and row, or column, row, and site.")
    ...
    def set_value(self, value: float, col: int, row: int) -> None:
        ...
        Args:
            ...
            row: The row of the die.
        """
        self.comm.send(f"map:bins:set_value {value}, {col}, {row}")
        Response.check_resp(self.comm.read_line())

    def set_bin_info(self, index: int, description: str, bin_quality: BinQuality, color: str) -> None:
        """Set bin information in the binning table.

        Wraps SENTIO's map:bins:set_bin_info remote command.
    ...
    def resize(self, bin_table_size: int) -> None:
        """Resize the binning table.

        Wraps SENTIO's map:bins:resize remote command.

        Args:
            bin_table_size: The new size of the binning table.
        """
        self.comm.send(f"map:bins:resize {bin_table_size}")
        Response.check_resp(self.comm.read_line())