...
class VisionCommandGroup(ModuleCommandGroupBase):
    ...
    def align_die(self, threshold: float = 0.05) -> Tuple[float, float, float]:
        ...
        self._comm.send(f"vis:align_die {threshold}")
    ...
    def detect_probepads(self, imgPath: str = None, minScore:float=0.7, startAngle:float=None, startExtend:float=None,maxOverlap:float=None) -> Tuple[bool, float, str]:
        if startAngle==None:
